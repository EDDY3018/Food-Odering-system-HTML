...
db.collection("items")
    ...
    .then(snapshot => {
       ...
       <h4 class="pt-2 baidya">GHc ${price}</h4>
...
function addTocart(seletedItem) {
    ...
    <div class="col-sm">GHc ${item.price}</div>
...
function renderItemModal(title, price, img) {
    if (title === undefined && price === undefined && img === undefined) {} else {
        let seletedItem = { title, price, img };
        modalTitle.innerHTML = title;
        modalPrice.innerHTML = `GHc ${price}`;
